...
import com.dsi.ant.antplusdemo.R;
...
public class AntPlusManager {
    ...
    private AntInterface sAntReceiver;
 ...
 // Variables to keep track of the status of each sensor
     ...
     * Default Constructor
     */
    public AntPlusManager(Activity activity, Bundle savedInstanceState, Context context, Callbacks callbacks)
    {
        ...
        {
            sAntReceiver = (AntInterface) mActivity.getLastNonConfigurationInstance();
            ...
            if(null == sAntReceiver)
            {
                ...
                Log.e(TAG, "AntChannelManager Constructor: No ANT Service, quitting.");
            ...
            else
            {
                ...
                mActivity.registerReceiver(mAntStatusReceiver, statusIntentFilter);
    ...
    public boolean checkAntState()
    {
        try
        {
            ...
            else if(mServiceConnected && sAntReceiver.isEnabled())
            {
                if(sAntReceiver.hasClaimedInterface() || sAntReceiver.claimInterface())
                {
                    return true;
                }
                else
    ...
    public void tryClaimAnt()
    {
        try
        {
            sAntReceiver.requestForceClaimInterface(mActivity.getResources().getString(R.string.app_name));
        }
        catch(AntInterfaceException e)
        {
            antError();
    ...
    public void shutDown()
    {
        try
        {
            mActivity.unregisterReceiver(mAntStatusReceiver);
        }
        ...
        try
        {
           ...
           {
              Log.d(TAG, "AntChannelManager.shutDown: isFinishing");
    ...
    public void closeChannel(byte channel)
    {
        ...
        try
        {
           sAntReceiver.ANTCloseChannel(channel);
           sAntReceiver.ANTUnassignChannel(channel);
        } catch (AntInterfaceException e)
        {
    ...
    private void receiveAntRxMessages(boolean register)
    {
        if(register)
        {
            ...
            mActivity.registerReceiver(mAntMessageReceiver, messageIntentFilter);
        }
        else
        {
            try
            {
                mActivity.unregisterReceiver(mAntMessageReceiver);
            }
            catch(IllegalArgumentException e)
            {
                // Receiver wasn't registered, ignore as that's what we wanted anyway
